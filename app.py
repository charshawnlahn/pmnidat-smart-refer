...
with st.sidebar:
    ...
    with st.expander("🟢 STEP 1: ระบบผู้ป่วยใน (IPD)", expanded=True):
        ...
        **1.2 การวินิจฉัย:**
        - กดเปิด "การวินิจฉัย" [cite: 5]
        - คัดลอกรหัส ICD-10 ชื่อโรค และประเภท (Principal & Comorbidity) [cite: 5]
    ...
    with st.expander("🔵 STEP 2: การประเมิน (Assessment)"):
        st.markdown("""
        - กดเมนู "Admission note" → กดปุ่ม **"ข้อมูลผู้ป่วยนอก"** [cite: 7]
        - เลื่อนลงล่างและกดที่หัวข้อ **Assessment** [cite: 7]
        - คัดลอกผลคะแนน **9Q, 8Q, BPRS** ทั้งหมดมาวาง [cite: 7]
        """)
    ...
    with st.expander("🟠 STEP 3: เวชระเบียน (Registration)"):
        ...
        **3.2 ทั่วไป 2:** - กดแสดง **"ที่อยู่ปัจจุบัน"** แล้วคัดลอกทั้งหมด [cite: 8]
        
        **3.3 ผู้ติดต่อ:** - คัดลอกชื่อญาติ, ความสัมพันธ์ และเบอร์โทร 
        
        **3.4 สิทธิรักษา:** - คัดลอกสิทธิ์และ **"สถานพยาบาลหลัก"** [cite: 9]
        """)
...
st.divider()
# กลุ่มที่ 1: ข้อมูลระบบผู้ป่วยใน (IPD) - เน้นการคัดลอกข้อมูลดิบแบบจัดเต็ม [cite: 71-72]
...
st.divider()
# กลุ่มที่ 2: การประเมิน (Assessment) - ดึงข้อมูลคะแนนสุขภาพจิต [cite: 73-74]
...
st.divider()
# กลุ่มที่ 3: ข้อมูลเวชระเบียน (Registration) - ข้อมูลระบุตัวตนและที่อยู่ [cite: 75-76]
...
if st.button("🚀 กดเพื่อประมวลผลและสกัดข้อมูลด้วย Gemini 3 Flash", use_container_width=True):
    ...
    # รวบรวมข้อมูลดิบจาก 9 ช่องที่กรอกไว้ในส่วนที่ 3 [cite: 71-76]
    all_raw_data = f"""
    ...
    with st.spinner('Gemini 3 Flash กำลังวิเคราะห์ข้อมูลเชิงลึกและคำนวณระยะเวลาในชุมชน...'):
        # ตรรกะการประมวลผลระดับ PhD พร้อมกฎการตัดขยะข้อมูล [cite: 80-82]
        prompt = f"""
        ...
        1. กฎการตัดขยะข้อมูล (Noise Reduction Rule): 
           Ignore (ละทิ้ง) ข้อมูล Theme Customizer, Navbar, Menu Colors, Light/Dark Mode และ COPYRIGHT ทั้งหมด [cite: 82]
        ...
        2. ตรรกะการสกัดและคำนวณข้อมูลพิเศษ (PhD Computation Logic):
           ...
           - [รวมวันนอนในโรงพยาบาล (LOS)]: บวกตัวเลขหน้าคำว่า 'วัน' จากแถว 'Detox' และ 'Rehab' [cite: 72]

        3. ตรรกะการสกัดข้อมูลรายหัวข้อ (Search & Extract Logic):
           - [HN]: มองหาตัวเลขหลัง 'HN' หรือ 'Hospital number' [cite: 72]
           - [สิทธิการรักษา (RIGHTS)]: สกัดข้อความหลัง 'สิทธิ์ :' [cite: 72]
           - [อาการนำส่ง (CC)]: สกัดจาก 'Chief Complaint' ถึง 'Present illness' [cite: 72]
           - [คะแนนประเมิน]: สกัดตัวเลขหลัง 'ซึมเศร้า' (9Q) และ 'ฆ่าตัวตาย' (8Q) [cite: 74]
           - [สรุปปัญหา (PROGRESS)]: สังเคราะห์จาก Progress Note ทั้งหมด ให้เป็นย่อหน้าเดียว ความยาว 2-3 บรรทัด [cite: 72, 79]

        4. นโยบายการตรวจสอบความถูกต้อง (Verification Audit Policy):
           - หากไม่พบข้อมูลใดๆ ให้พยายามหาจากช่องอื่น หากไม่มีจริงๆ ให้ระบุ [กรอกด้วยตนเอง] [cite: 78]
