...
# --- 9. ฟังก์ชันจัดการไฟล์ Word (วันที่ชิดขวา + ส่วนอื่นชิดซ้าย + ฟอนต์ 13) ---

def fill_pmnidat_doc(data):
    """ฟังก์ชันนำข้อมูลจาก JSON ไปบรรจุลงในไฟล์แม่แบบ .docx [cite: 8-10, 14]"""
    try:
        # โหลดไฟล์แม่แบบที่คุณหมอเตรียมไว้
        doc = Document("PMNIDAT 062 แบบบันทึกข้อมูลเพื่อส่งต่อ.docx")
        
        # เตรียมชุดข้อมูล Mapping (Key ตัวพิมพ์ใหญ่) [cite: 18-34]
        mapping = {f"{{{{{k.upper()}}}}}": str(v) for k, v in data.items()}
        
        def apply_style_and_replace(paragraph):
            """ตรรกะการแทนที่พร้อมจัดรูปแบบตามเงื่อนไขของคุณหมออาร์ม"""
            is_date_line = False
            # ตรวจสอบว่าในย่อหน้านี้มี Placeholder ของวันที่หรือไม่
            if any(k in paragraph.text for k in ["{{DAY}}", "{{MONTH}}", "{{YEAR}}"]):
                is_date_line = True

            for key, value in mapping.items():
                if key in paragraph.text:
                    # แทนที่ Placeholder ด้วยข้อมูลจริง [cite: 18-34]
                    paragraph.text = paragraph.text.replace(key, value)
                    
                    # บังคับการจัดวาง: หากเป็นบรรทัดวันที่ให้ชิดขวา นอกจากนั้นให้ชิดซ้าย
                    if is_date_line:
                        paragraph.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                    else:
                        paragraph.alignment = WD_ALIGN_PARAGRAPH.LEFT
                    
                    # ปรับขนาดฟอนต์เป็น 13 pt สำหรับทุกจุดที่แทนที่
                    for run in paragraph.runs:
                        run.font.size = Pt(13)

        # ดำเนินการในส่วนเนื้อหาหลักและในตาราง [cite: 14-43]
...
if "extracted_json_data" in st.session_state and st.session_state.extracted_json_data:
    # สร้างไฟล์ Word พร้อมตรรกะจัดวางแบบใหม่
    ...
    if word_file_final:
        ...
        st.divider()
        st.balloons() 
        st.success("🎉 ระบบจัดทำเอกสาร PMNIDAT 062 เรียบร้อยแล้ว (วันที่ชิดขวาตามสั่ง)!")
...
st.info("""
    ...
    * ข้อมูลจะถูกลบทิ้งทันทีเมื่อปิดเบราว์เซอร์หรือกด Refresh หน้าจอ
    * **คำแนะนำ:** โปรดตรวจสอบความถูกต้องของข้อมูล (Verification Audit) อีกครั้งก่อนใช้งานจริง
    """)
