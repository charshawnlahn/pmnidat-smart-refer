...
with st.sidebar:
    ...
    **ขั้นตอนง่ายๆ สำหรับพี่พยาบาล:**
    1. ลากเมาส์คลุมข้อความในระบบ @ThanHIS
    2. กด **Ctrl+C** (คัดลอก)
    3. มาที่หน้านี้ คลิกในช่องแล้วกด **Ctrl+V** (วาง)
    """)
    
    st.markdown("""
    **🟢 STEP 1: ระบบผู้ป่วยใน (IPD)**
    1. **Admission Note:** หน้าแรกรับ → คัดลอกทั้งหมด
    2. **การวินิจฉัย:** หน้าวินิจฉัย → คัดลอกทั้งหมด
    3. **Order/Meds:** หน้า Order → คัดลอก Discharge order
    4. **Progress Note:** หน้า SOAP ล่าสุด → คัดลอกทั้งหมด
    
    **🔵 STEP 2: การประเมิน**
    * หน้า Admission → ปุ่ม 'ข้อมูลผู้ป่วยนอก' → Assessment → คัดลอก 9Q, 8Q, BPRS
    
    **🟠 STEP 3: เวชระเบียน (Registration)**
    * หน้าลงทะเบียน → 'ทั่วไป 1', 'ที่อยู่ 2.2', 'ผู้ติดต่อ' และ 'สิทธิรักษา'
    """)
    st.divider()
    st.success("💡 AI โดยหมอชาฌาน จะจัดรูปแบบยาและโรคให้สวยงามอัตโนมัติครับ")
