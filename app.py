...
# --- 1. การตั้งค่าการเชื่อมต่อ (แก้ไขชื่อโมเดลเป็นรุ่นล่าสุดปี 2026) ---
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
    APPS_SCRIPT_URL = st.secrets["APPS_SCRIPT_URL"]
    
    # ใช้ Library google-genai มาตรฐานใหม่
    client = genai.Client(api_key=API_KEY)
    
    # แก้ไข ID เป็น gemini-3.0-flash ตามที่ปรากฏใน Paid Tier 1 Dashboard
        ...
        def apply_style_and_replace(paragraph):
            for key, value in mapping.items():
                if key in paragraph.text:
                    ...
                    # บังคับชิดขวา เพื่อแก้ปัญหาข้อความยืด (Thai Distributed)
...
# --- 4. การออกแบบหน้าเว็บและคู่มือแบบละเอียด ---
...
with st.sidebar:
    ...
    st.info("""
    **ขั้นตอนง่ายๆ สำหรับพี่พยาบาล:** คัดลอกข้อมูลจากระบบ @ThanHIS มาวาง
    1. คลิ๊กเมาส์ซ้ายค้างที่ต้นข้อความ ลากครอบให้หมด
    ...
    * ข้อมูลทั่วไป, ที่อยู่ปัจจุบัน, ผู้ติดต่อ และสิทธิการรักษา
    """)
    st.divider()
    st.success("💡 ระบบทำงานด้วย Gemini 3.0 Flash (Paid Tier 1)")

st.title("🏥 PMNIDAT Smart D/C Transfer")
st.subheader(f"ระบบสร้างไฟล์ใบส่งต่ออัตโนมัติ (ขุมพลัง: {MODEL_ID})")
...
if st.button("🚀 สกัดข้อมูลและสร้างเอกสาร"):
    ...
    with st.spinner(f'กำลังประมวลผลด้วย {MODEL_ID}...'):
        prompt = f"""
...
st.divider()
st.info("ประกาศ: มาตรการรักษาความปลอดภัยข้อมูล (PDPA) - ระบบไม่มีการจัดเก็บข้อมูลผู้ป่วยถาวร")
