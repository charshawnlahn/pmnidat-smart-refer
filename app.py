...
def load_test_data():
    # ข้อมูลจาก Step 1.1
    st.session_state.s11 = "นาย ส. ธัญญารักษ์ อายุ 40 ปี 5 เดือน\nสิทธิ์ : จ่ายตรงกรมบัญชีกลาง\nAdmit Date 01/03/2569\nจำนวนวัน Detox [5] วัน Rehab [10] วัน\nCC : เสพยาบ้าซ้ำ ต้องการเข้ารับการบำบัดรักษา"
    # ข้อมูลจาก Step 1.2 (รหัสโรคพร้อมชื่อไทย)
    st.session_state.s12 = "1. F102 - โรคติดสุรา (Alcohol dependence)\n2. I10 - โรคความดันโลหิตสูง (Hypertension)"
    ...
    # ข้อมูลจาก Step 3.1
    st.session_state.s31 = "ชื่อ [ส.] นามสกุล [ธัญญารักษ์]\nเลขบัตรประชาชน* [1-2345-67890-12-3]\nศาสนา [พุทธ] อาชีพ [ข้าราชการ]"
    # ข้อมูลจาก Step 3.2
    st.session_state.s32 = "ที่อยู่ปัจจุบัน: เลขที่ 60 ต.ประชาธิปัตย์ อ.เมืองปทุมธานี จ.ปทุมธานี"
    # ข้อมูลจาก Step 3.3
    st.session_state.s33 = "คุณ ญ. ธัญญารักษ์ (ภรรยา) เบอร์โทร: 081-234-5678"
    # ข้อมูลจาก Step 3.4
...
st.subheader("ระบบกรอกข้อมูลลง "แบบบันทึกข้อมูลเพื่อส่งต่อ (PMNIDAT 062)" โดยอัตโนมัติ (Master Version 3.32)")
...
with st.sidebar:
    ...
    # คำแนะนำพื้นฐานในการคัดลอกและวางข้อมูล
    st.markdown("### **วิธีการคัดลอกและวางข้อมูลจาก @ThanHIS**")
    st.info("""
    1. **คลิกเมาส์ซ้ายค้าง** ที่ต้นข้อความ แล้วลากเมาส์ลงไปล่างสุดให้ครอบคลุมข้อมูลทั้งหมด
    2. คัดลอกข้อมูล โดยกด **Ctrl+C** (ใช้วิธีคลิกขวาแล้วกด copy ก็ได้เช่นกัน)
    3. มาที่หน้านี้ เลือกช่องที่ต้องการวางข้อมูล แล้วกด **Ctrl+V** (ใช้วิธีคลิกขวาแล้วกด paste ก็ได้เช่นกัน)
    """)
    ...
    with st.expander("🟢 STEP 1: ระบบผู้ป่วยใน (IPD)", expanded=True):
        st.markdown("""
        **1.1 Admission Note:**
        - กดดูข้อมูลคนไข้ → หน้า Admission note
        - คลิกเมาส์ซ้ายค้างแล้วลากเพื่อคัดลอกข้อมูลทั้งหมด 
        
        **1.2 การวินิจฉัย:**
        - ในหน้า Admission note เดิม ให้กดเปิด "การวินิจฉัย"
        - คัดลอกการวินิจฉัยทั้งหมด 
        
        **1.3 Order / Meds:**
        - กดเมนู **"Order"** 
        - ลากเพื่อคัดลอก **Order** ทั้งหมด
        
        **1.4 Progress Note:**
        - กดเมนู **"Progress note"** ด้านซ้าย
        - คัดลอกบันทึก **Progress note** ทั้งหมด
        """)
