...
# --- 9. ฟังก์ชันจัดการไฟล์ Word (Smart Alignment & PhD Formatting) ---

def fill_pmnidat_doc(data):
    """ฟังก์ชันนำข้อมูลลงไฟล์แม่แบบ พร้อมระบบจัดช่องไฟอัตโนมัติ [cite: 35-49]"""
    try:
        doc = Document("PMNIDAT 062 แบบบันทึกข้อมูลเพื่อส่งต่อ.docx")
        
        # เตรียมชุดข้อมูลสำหรับแทนที่
        mapping = {f"{{{{{k.upper()}}}}}": str(v) for k, v in data.items()}
        
        def apply_style_and_replace(paragraph):
            """ตรรกะจัดรูปแบบ: วันที่ชิดขวา / ช่องไฟในแถว / หัวข้อตัวหนา"""
            
            # 1. การจัดการการจัดวาง (Alignment Logic)
            # วันที่หัวเอกสาร (ที่ไม่มีคำว่าวันที่จำหน่าย) ให้ชิดขวา [cite: 32]
            if any(k in paragraph.text for k in ["{{DAY}}", "{{MONTH}}", "{{YEAR}}"]) and "วันที่จำหน่าย" not in paragraph.text:
                paragraph.alignment = WD_ALIGN_PARAGRAPH.RIGHT
            else:
                # ส่วนอื่นรวมถึง 'วันที่จำหน่าย' ให้ชิดซ้ายตามปกติ [cite: 44]
                paragraph.alignment = WD_ALIGN_PARAGRAPH.LEFT

            # 2. การจัดการช่องไฟในแถวเดียว (Row Spacing Logic)
            # หากเป็นแถวที่มีข้อมูลหลายตัว ให้ฉีด Tab (\t) เข้าไปเพื่อกระจายข้อมูล
            multi_item_rows = ["ชื่อ-สกุล", "เลขบัตรประชาชน", "อาชีพ", "วันที่จำหน่ายครั้งสุดท้าย"]
            has_multi_items = any(h in paragraph.text for h in multi_item_rows)

            for key, value in mapping.items():
                if key in paragraph.text:
                    # แทนที่ Placeholder พร้อมเพิ่มช่องไฟ (\t) หากเป็นแถวที่มีข้อมูลหลายชุด [cite: 35-40]
                    replacement_value = value + "\t" if has_multi_items else value
                    paragraph.text = paragraph.text.replace(key, replacement_value)
                    
                    # 3. ตรรกะการเน้นตัวหนา (Bolding) สำหรับหัวข้อทางคลินิก [cite: 48-50]
                    clinical_headers = ["Home Medication", "การวินิจฉัย", "สรุปปัญหา"]
        ...
        # ประมวลผลทั้งในย่อหน้าและตาราง [cite: 31-60]
        ...
        st.error(f"⚠️ เกิดข้อผิดพลาดในการจัดการไฟล์: {e}")
        return None

# --- 10. การแสดงผลลัพธ์และการควบคุมการดาวน์โหลด (Final Execution) ---

# ตรวจสอบว่ามีการสกัดข้อมูลสำเร็จและเก็บไว้ใน Session State หรือไม่
if "extracted_json_data" in st.session_state and st.session_state.extracted_json_data:
    # เรียกใช้ฟังก์ชันจัดการไฟล์ Word พร้อมระบบจัดช่องไฟ (Smart Spacing) จากส่วนที่ 6
    word_file_final = fill_pmnidat_doc(st.session_state.extracted_json_data)
    
    if word_file_final:
        # บันทึกสถิติการใช้งานไปยังระบบ Log (อ้างอิงฟังก์ชันในส่วนที่ 1)
        log_usage(st.session_state.extracted_json_data.get('NAME', '[ไม่ระบุชื่อ]'))
        
        st.divider()
        st.balloons() # แสดงเอฟเฟกต์เฉลิมฉลองความสำเร็จ
        st.success("🎉 ระบบจัดทำเอกสาร PMNIDAT 062 ฉบับสมบูรณ์ (Version 3.35) เรียบร้อยแล้ว!")
        
        # ปุ่มดาวน์โหลดไฟล์เอกสารที่จัดรูปแบบฟอนต์ 13 และชิดขวาตามระเบียบสารบรรณ
        st.download_button(
...
# --- 11. มาตรการรักษาความปลอดภัยและความเป็นส่วนตัว (PDPA Compliance) ---
# ส่วนประกาศความปลอดภัยที่คุณหมออาร์มกำหนดเพื่อรักษาความลับของผู้ป่วยตามมาตรฐานสากล

st.divider()
st.info("""
    **🛡️ มาตรการรักษาความปลอดภัยของข้อมูลคนไข้ (PDPA Compliance):**
    
    * **ความปลอดภัยของข้อมูล:** ระบบ PMNIDAT Smart Transfer ออกแบบมาเพื่อประมวลผลแบบ Real-time บนหน่วยความจำชั่วคราว (RAM) เท่านั้น **ไม่มีการจัดเก็บข้อมูลส่วนบุคคลของผู้ป่วย** ลงในฐานข้อมูลถาวรหรือบันทึกไฟล์ทิ้งไว้บนเซิร์ฟเวอร์
    ...
    * **การตรวจสอบความถูกต้อง (Verification Audit):** เนื่องจากเป็นการใช้ระบบ AI ช่วยสกัดข้อมูล ผู้ใช้งานต้องตรวจสอบความถูกต้องของข้อมูลและลงนามรับรอง [cite: 58] ในเอกสารฉบับจริงทุกครั้งก่อนนำไปใช้งานทางคลินิก
    
    *พัฒนาระบบโดย: ดร.นพ.ชาฌาน หลานวงศ์* 
    """)
