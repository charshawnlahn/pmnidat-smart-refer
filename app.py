...
with st.sidebar:
    ...
    **ขั้นตอนง่ายๆ สำหรับพี่พยาบาล:วิธีคัดลอกข้อมูลจากระบบ @ThanHIS มาวางตามช่องย่อย**
    ...
    2. จากนั้นคลิ๊กขวา เลือก copy หรือจะกด **Ctrl+C** (คัดลอก) ก็ได้ครับ
    3. มาที่จอหน้านี้ กดที่ช่องย่อยที่จะวาง จากนั้นคลิ๊กขวา เลือก paste หรือกด **Ctrl+V** (วาง) ก็ได้ครับ
    """)
    
    st.markdown("""
    **🟢 STEP 1: ระบบผู้ป่วยใน (IPD)**
    1. **Admission Note:** กดดูข้อมูลคนไข้ → Admission note → คัดลอกมาทั้งหมด
    2. **การวินิจฉัย:**  กด "การวินิจฉัย" → คัดลอกมาทั้งหมด
    3. **Order/Meds:** หน้า "Order" → คัดลอกมาทั้งหมด
    4. **Progress Note:** หน้า "Progress note" → คัดลอกมาทั้งหมด
    ...
    * กดหน้า **ทั่วไป 1** →  คัดลอก
    * กดหน้า **ทั่วไป 2** →  กดแสดง "ทั่วไป 2.2 (ที่อยู่ปัจจุบัน)" → คัดลอก
    * กดหน้า **ผู้ติดต่อ** → คัดลอก
    * กดหน้า **สิทธิการรักษา** → คัดลอก
    """)
    st.divider()
    st.success("💡 AI จะจัดรูปแบบข้อมูลให้สวยงามแบบแบบฟอร์มอัตโนมัติครับ")
